...
2. 調査・原稿: 一次情報、記事brief、無料パブリシティ候補を調査する。公開原稿は利用可能な最新モデルのサブエージェントで作り、統括が事実、簡潔さ、法務、重複、導線を確認する。短く、簡単な言葉と自然な文脈で伝える。
3. 実装・検証・自己分析: 局所実装案、既存デザイン適合、アクセシビリティ、SEO、表示、回帰、進め方をレビューする。

production write、最終統合、commit、push、deploy、状態更新、automation更新は統括だけが行います。
...
既存UI変更では現在のデザインシステムを使い、Lazywebで実在例を確認します。カードを多用せず、余白、タイポグラフィ、罫線、一覧性を中心にします。不要なpを増やさず、意味に応じて見出し、リスト、brを使います。指定がない限り「、」でbr改行しません。広いリデザイン、ブランドカラー、動き、ヘッダー構造の置換は行いません。
